...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public Model.WorkInfoNo GetModel(int id)
        {
            ...
            {
                string sql = "select workinfo.*,activity.activityName from workinfo join activity on workinfo.activityId=activity.activityId where work Id=@id";
        ...
        public IEnumerable<Model.WorkInfoNo> GetFindCount(Model.WorkFindPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
        ...
        public int Update(Model.WorkInfo workinfo)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "update workinfo set workName=@workname,workPicture=@workPicture,uploadTime=@uploadTime,workIntroduction=@workIntroduction,workVerify=@workVerify," + "userName=@userName,recomment=@recomment,recommendTime=@recommendTime where workId=@workId";
